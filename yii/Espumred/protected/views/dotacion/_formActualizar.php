   ...
   //          $(this).parent().remove();
    ...
    function cargarPagina() {//funcio para cargar pagina cuando se devuelve
          event.preventDefault();
                      ...
                      var delantal= $("#delantal").val();
                      var overol= $("#overol").val();
                      var otrasDotaciones= $("#otrasDotaciones").val();
                      
                  var Registros;
                   $.post("<?php echo Yii::app()->request->baseUrl ?>/index.php?r=dotacion/cargarActualizar",{camisa:camisa,calzado:calzado,pantalon:pantalon, delantal:delantal,overol:overol,otrasDotaciones:otrasDotaciones},
                        function(data) {
                          console.log(data);
                          var Registro = data;
                          
                            Registros = $.parseJSON(Registro);
                            document.getElementById("camisa").value=Registros[0];
                            document.getElementById("calzado").value=Registros[1];
                            document.getElementById("pantalon").value=Registros[2];
                            document.getElementById("delantal").value=Registros[3];
                            document.getElementById("overol").value=Registros[4];
                            document.getElementById("otrasDotaciones").value=Registros[5];
